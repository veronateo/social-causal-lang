...
class Semantics:
    """
    Computes semantic truth values ⟦u⟧(w) for each causal expression
    given a world state w = (C, A, V).
    
    Returns a normalized probability distribution over verbs,
    with epsilon smoothing for numerical stability.
    """
    def __init__(self, 
                 epsilon: float = 0.01, 
    ...
    def get_verb_probabilities(self, state: WorldState) -> Dict[str, float]:
        """
        Compute semantic truth values ⟦u⟧(w) and return normalized distribution.

        Args:
            state: WorldState with fields C (necessity), A (acted), V (aligned).
        """
        C = float(state.necessity)   # Counterfactual necessity
...
InertialVerbClassifier = Semantics
