...
from .physical_inference import PreferenceInference, softmax_choice
...
class PhysicalDomain(Domain):
    ...
    def compute_dependence(self, trial_data: Dict[str, Any], posterior: Dict[float, float]) -> Dict[str, float]:
        ...
        for action in possible_actions:
            ...
            for theta, theta_prob in posterior.items():
                ...
                # Softmax prob of choosing right
        ...
        # Max dependence (vs best alternative)
        nec_max = max(action_diff_probs.values())
        
        # Average dependence
        # Exclude the actual action taken
    ...
    def get_domain_state(self, trial_data: Dict[str, Any], theta: Optional[float] = None) -> WorldState:
        ...
        # Expected outcome based on initial direction 
        initial_direction = trial_data['farmer_initial_direction']
        expected_outcome = 'apple' if initial_direction == 'right' else 'banana'
        
        # V — Value alignment: does outcome match farmer's values / preferences?
