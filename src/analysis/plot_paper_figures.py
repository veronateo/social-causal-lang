...
from src.analysis.speaker import normalize_response_counts, bootstrap_human_data, get_error_bars
from src.utils.plotting import colors as standard_colors
from src.analysis.plotting_utils import COLORS as VERB_COLORS, MODEL_COLORS
...
class HandlerHeader(HandlerBase):
    def create_artists(self, legend, orig_handle,
                       ...
                       trans):
        # Draw the text at x=0 (left-aligned with where handles start)
        # We use a bit of bolding for the header
...
def load_predictions(csv_path: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            # Simplified: No trial_data needed for these bar plots
...
def plot_figure_1(all_results, domain_errors):
    """
    Figure 1: Specific Trial Comparison
    2x3 Grid
    Physical: C, E
    Belief: D, E
    Preference: G, D
    ...
    for domain, tid, ax, title in targets:
        ...
        # domain_errors[domain] is the DataFrame for that domain
        # We need to construct the full trial ID (e.g. trial_c)
        full_tid = tid if tid.startswith('trial_') else f"trial_{tid}"
        
        # get_error_bars returns (lower_errors, upper_errors) arrays
        yerr_lower, yerr_upper = get_error_bars(full_tid, VERBS, np.array(human_vals), domain_errors.get(domain))

        # Plot bars per verb to get correct colors
        for i, v in enumerate(VERBS):
            ...
            # 2. Hatch with solid color (no fill)
...
def plot_scatter_comparison(all_results, domain_errors):
    """
    Figure 3: Scatter plots of Human vs Model predictions
    1x4 Row: Full, No Causal, No Preference, Semantics
    Each point is a (Trial, Verb) pair.
    Model on X-axis, Human on Y-axis.
    Includes 95% CIs on Human judgments.
    """
    ...
    for domain, domain_trials in all_results.items():
        ...
        for trial in domain_trials:
            N = max(trial.get('human_n', 0), 1) # Avoid div by zero
            # Construct full tid
            tid = trial['trial_id']
            full_tid = tid if tid.startswith('trial_') else f"trial_{tid}"
    ...
    # Side Legend (Single Column)
    from matplotlib.lines import Line2D
    
    # Use HeaderItem and HandlerHeader for left-aligned titles
    ...
    # Build list of handles and labels
    # For headers, the label is '' because the HandlerHeader draws the text itself
    ...
    # Legend to the right of the last subplot
...
def generate_overview_figure(all_results, domain_errors):
    """
    Figure 4: Three Trials Overview (Empirical Only)
    Uses pre-loaded all_results and pre-calculated domain_errors.
    """
    # Define the trials to plot
    specs = [
        {"name": "Physical", "trial": "trial_a", "label": "Physical Domain (Trial A)"},
        {"name": "Belief", "trial": "trial_e", "label": "Belief Domain (Trial E)"},
        {"name": "Preference", "trial": "trial_e", "label": "Preference Domain (Trial E)"}
    ]
    
    # X-axis labels (Abbreviations)
    short_labels = ["C", "E", "A", "N"]

    verbs = ['caused', 'enabled', 'allowed', 'made_no_difference']
    verb_labels = short_labels
    verb_colors = [VERB_COLORS[v] for v in verbs]
    
    # Adjust figsize
    fig, axes = plt.subplots(3, 1, figsize=(2.5, 5.5))
    ...
    for i, spec in enumerate(specs):
        ...
        # Get trial data from all_results
        trial = get_trial(all_results[domain_key], trial_id)
        if not trial:
            print(f"Warning: Trial {trial_id} not found in {domain_key}")
            continue
            
        # Extract proportions (human)
        # props needs to be list of values in order of 'verbs'
        props_values = [trial['human'].get(v, 0.0) for v in verbs]
    
        full_tid = trial['trial_id']
        
        yerr_lower, yerr_upper = get_error_bars(full_tid, verbs, np.array(props_values), domain_errors.get(domain_key))
        
        # Plot bars (Human only)
        # Use full color, no hatches
        ax.bar(range(len(verbs)), props_values, color=verb_colors, width=0.7,
               yerr=[[yerr_lower[j] for j in range(4)], [yerr_upper[j] for j in range(4)]] if yerr_lower is not None else None,
               ecolor='#5C5C5C', capsize=2, error_kw={'elinewidth': 0.8})
    ...
    # Save
    # Save to OUTPUT_DIR
    output_path = os.path.join(OUTPUT_DIR, "three_trials_plot.png")
...
def main():
    ...
    if os.path.exists(PREDICTIONS_CSV):
        ...
        # Load and bootstrap human data for all domains (shared error bars)
        domain_errors = load_and_bootstrap_all_domains()
        
        print("Generating specific trials figure 1...")
        plot_figure_1(all_results, domain_errors)
        
        print("Generating scatter comparison figure 3...")
        plot_scatter_comparison(all_results, domain_errors)
        
        # Note: generate_overview_figure loads its own data but uses same bootstrap method/seed.
        print("Generating overview figure 4...")
        generate_overview_figure(all_results, domain_errors)
