...
const PHYSICAL_LISTENER_GENERATIVE_ID = "qFE0D8dRxEGw";
const PHYSICAL_SPEAKER_ID = "AqdxpB7wOgK5";

let experiment_id;
let isListenerExperiment = false;
let isGenerativeListenerExperiment = false;
...
} else if (path.includes('/physical/listener-generative')) {
    experiment_id = PHYSICAL_LISTENER_GENERATIVE_ID;
    isGenerativeListenerExperiment = true;
    // console.log('[DataPipe] Mode: Physical Listener Generative');
...
const save_trials = {
    ...
    data_string: () => {
        ...
        } else if (isGenerativeListenerExperiment) {
            // Generative Listener experiment
            const speakerData = jsPsych.data.get().filter({ trial_type: 'gridworld-trial' });
            const genData = jsPsych.data.get().filter({ trial_type: 'generative-listener-trial' });

            let csv = 'task,trial_number,verb,outcome,utterance,rock_present,farmer_initial_dir,wizard_action,farmer_final_dir,matched_trial_id,rt\n';

            for (let trial of speakerData.values()) {
                csv += `${escapeCSV(trial.task)},${escapeCSV(trial.trial_id)},,,,,,,,${escapeCSV(trial.response)},${escapeCSV(trial.rt)}\n`;
            }

            for (let trial of genData.values()) {
                csv += `${escapeCSV(trial.task)},${escapeCSV(trial.trial_number)},${escapeCSV(trial.verb)},${escapeCSV(trial.outcome)},${escapeCSV(trial.utterance)},${escapeCSV(trial.rock_present)},${escapeCSV(trial.farmer_initial_dir)},${escapeCSV(trial.wizard_action)},${escapeCSV(trial.farmer_final_dir)},${escapeCSV(trial.matched_trial_id)},${escapeCSV(trial.rt)}\n`;
            }
